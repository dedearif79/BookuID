...
Public Class frm_GantiTahunBuku

    Public FungsiForm
    Public JudulForm

    Private Sub frm_GantiTahunBuku_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Select Case FungsiForm
            Case FungsiForm_MASUKTAHUNBUKU
                ...
                btn_Ganti.Text = "Masuk"
                'AksesDatabase_General(Buka)
                'cmd = New OdbcCommand("SELECT * FROM tbl_InfoData WHERE Tahun_Buku = '" & TahunIni & "' ", KoneksiDatabaseGeneral)
                'dr = cmd.ExecuteReader
                'If Not dr.HasRows Then
                '    Dim PilihBuatTahunBuku = MessageBox.Show("Anda belum membuat Database Tahun Buku " & TahunIni & Enter2Baris & "Buat Database Tahun Buku " & TahunIni & " sekarang..?", "Perhatian..!", MessageBoxButtons.YesNo)
                '    If PilihBuatTahunBuku = vbYes Then
                '        AksesDatabase_General(Tutup)
                '        frm_BuatDatabaseBukuBaru.ShowDialog()
                '        Me.Close()
                '    End If
                'End If
                'AksesDatabase_General(Tutup)
    ...
    Sub KontenComboTahunBuku()
        ...
        Loop
        cmb_TahunBuku.Text = ListTahunBuku
        AksesDatabase_General(Tutup)
    End Sub
    ...
    Public Sub ProsesGantiTahunBuku()
        ...
        BuatDsnGeneral() 'Kenapa mesti selalu memanggil sub ini..? Untuk antisipasi kemungkinan perubahan lokasi server database..! Dan juga ada beberapa value yang harus diisi di awal
        BuatDsnTransaksi(TahunBukuBaru) 'Kenapa mesti selalu memanggil sub ini..? Untuk antisipasi kemungkinan perubahan lokasi server database..!  Dan juga ada beberapa value yang harus diisi di awal
        ...
        If ProsesGantiTahun = True Then
            If TahunBukuAktif = TahunBukuTerakhirDibuka Then
                ...
                AksesDatabase_General(Tutup)
            Else
                TrialBalance_Mentahkan() 'Karena Update Saldo, Tentu Trial Balance Harus Dimentahkan Lagi...!!!
                AksesDatabase_General(Buka) '<--- Database General harus dikoneksi di sini, karena sudah diputus di Sub TrialBalance_Mentahkan.
        ...
        Else
            ...
            cmd.ExecuteNonQuery() 'Ini dibutuhkan untuk mengantisipasi data saldo yang tidak utuh di-update.
        End If
        AksesDatabase_General(Tutup)
        ...
        If ProsesGantiTahun = True Then
            If FungsiForm <> FungsiForm_EksekusiSub_PROSESGANTITAHUNBUKU Then PesanSukses("Anda memasuki Tahun Buku " & TahunBukuAktif & ".") 'Message ini harus disimpan di paling akhir sub.
        ...
        Else
            ...
            LoginGagal()
            Return
        End If '<---- Tidak boleh ada coding lagi setelah baris ini.

    End Sub

    Private Sub cmb_TahunBuku_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmb_TahunBuku.SelectedIndexChanged
    End Sub
    Private Sub cmb_TahunBuku_KeyPress(sender As Object, e As KeyPressEventArgs) Handles cmb_TahunBuku.KeyPress
        KunciTotalInputan(sender, e)
    End Sub

    Private Sub btn_Ganti_Click(sender As Object, e As EventArgs) Handles btn_Ganti.Click
        'Koding Sub ini hanya untuk Level User 81 (TimIT) ke atas.
        TahunBukuBaru = AmbilAngka(cmb_TahunBuku.Text)
        ProsesGantiTahunBuku()
        Me.Close()
    End Sub
    Sub TesKoneksiDatabaseTransaksiYangBaru()
        'Tes semua koneksi Database : (Ini jangan dilewatkan..! Penting..!)
        AksesDatabase_Transaksi(Buka)
        AksesDatabase_Transaksi(Tutup)
