...
Public Class wpfWin_PenerbitUpdater
    ...
    Private Sub wpfWin_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ...
        folderPathBooku = Path.Combine("D:\VB .Net Project\BookuID\Booku\bin\Debug\net8.0-windows\")
    ...
    Async Function KompressPaketBooku() As Task
        ...
        If ProsesPenerbitanUpdater Then
            SalinFolder(folderPathBooku, folderPathTempBooku)
            LogikaProsesPenerbitanUpdater(SalinFolderBerhasil)
        End If

        If ProsesPenerbitanUpdater Then
            HapusFolder(Path.Combine(folderPathTempBooku, "Attach"))
            LogikaProsesPenerbitanUpdater(HapusFolderBerhasil)
        End If

        If ProsesPenerbitanUpdater Then
            HapusFolder(Path.Combine(folderPathTempBooku, "Client"))
            LogikaProsesPenerbitanUpdater(HapusFolderBerhasil)
        End If
    ...
    Async Function UploadPaketBooku() As Task
        ...
        If ProsesPenerbitanUpdater Then
            Await UploadFileFTPAsync_MetodeHTTP(zipFilePathBooku, urlFolderServerBookuID_Support, urlFileUploader_PHP, pgb_UploadPaketBooku, lbl_UploadPaketBooku, 1, 100)
    ...
    Async Function UploadPaketUpdater() As Task
        ...
        If ProsesPenerbitanUpdater Then
            Await UploadFileFTPAsync_MetodeHTTP(zipFilePathUpdater, urlFolderServerBookuID_Support, urlFileUploader_PHP, pgb_UploadPaketUpdater, lbl_UploadPaketUpdater, 1, 100)
            LogikaProsesPenerbitanUpdater(UploadBerhasil)
        End If
