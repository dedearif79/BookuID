...
Public Class X_frm_POPembelian_X
    ...
    Sub ResetJudulForm()
        If Pilih_JenisProduk_Induk = JenisProduk_Semua Then
            JudulForm = "Purchase Order (PO)"
        Else
            JudulForm = "Purchase Order (PO) - " & JenisProduk_Menu
        End If
        Me.Text = JudulForm
    ...
    Sub TampilkanData()
        ...
        Do While dr.Read
            ...
            '---------- BARIS AWAL YANG NANTINYA HARUS DIHAPUS SETELAH PROSES TRANSISI SISTEM DATA PEMBELIAN -----------------
            'NomorSJBAST = Kosongan
            'TanggalSJBAST = Kosongan
            'NomorInvoice = Kosongan
            'TanggalInvoice = Kosongan
            'cmdTELUSUR = New OdbcCommand(" SELECT * FROM tbl_Pembelian " &
            '                             " WHERE PO_Nomor = '" & NomorPO & "'", KoneksiDatabaseTransaksi)
            'drTELUSUR_ExecuteReader()
            'Do While drTELUSUR.Read
            '    If NomorSJBAST = Kosongan Then
            '        NomorSJBAST = drTELUSUR.Item("Nomor_SJ_BAST")
            '        TanggalSJBAST = AmbilTanggal(drTELUSUR.Item("Tanggal_SJ_BAST"))
            '        NomorInvoice = drTELUSUR.Item("Nomor_Invoice")
            '        TanggalInvoice = AmbilTanggal(drTELUSUR.Item("Tanggal_Invoice"))
            '    Else
            '        NomorSJBAST &= DoubleSlash & Enter1Baris & drTELUSUR.Item("Nomor_SJ_BAST")
            '        TanggalSJBAST &= DoubleSlash & Enter1Baris & drTELUSUR.Item("Tanggal_SJ_BAST")
            '        NomorInvoice &= DoubleSlash & Enter1Baris & drTELUSUR.Item("Nomor_Invoice")
            '        TanggalInvoice &= DoubleSlash & Enter1Baris & AmbilTanggal(drTELUSUR.Item("Tanggal_Invoice"))
            '    End If
            'Loop '---------- BARIS AKHIR YANG NANTINYA HARUS DIHAPUS SETELAH PROSES TRANSISI SISTEM DATA PEMBELIAN -----------------
    ...
    Sub TambahBaris()
        ...
        DataTabelUtama.Rows.Add(NomorUrut, AngkaPO, NomorPO, TanggalPO, JenisProduk, KodeSupplier, NamaSupplier, Attention, KodeProject, PembuatPO,
                                ...
                                JenisPPN, PerlakuanPPN, PPN, TotalTagihan, TermOfPayment,
    ...
    Private Sub btn_Tambah_Click(sender As Object, e As EventArgs) Handles btn_Tambah.Click
        ...
        'frm_Input_POPembelian.ResetForm()
        'frm_Input_POPembelian.FungsiForm = FungsiForm_TAMBAH
        'frm_Input_POPembelian.JenisProduk_Induk = JenisProduk_Menu
        'frm_Input_POPembelian.ShowDialog()
    ...
    Private Sub btn_Edit_Click(sender As Object, e As EventArgs) Handles btn_Edit.Click
        ...
        End If
        win_InputPOPembelian.cmb_Kontrol.SelectedValue = Kontrol_Terseleksi
        IsiTabelSJBAST()
        IsiTabelInvoice()
    ...
    Sub IsiTabelProduk()
        ...
        Do While dr.Read
            ...
            frm_Input_POPembelian.DataTabelUtama.Rows.Add(NomorUrut, JenisProduk_PerItem, NamaProduk, DeskripsiProduk,
    ...
    Sub IsiTabelSJBAST()
        ...
        For Each KontenPerBaris In DataSJBAST
            ...
            If drTELUSUR.HasRows Then
                TanggalSJBAST = TanggalFormatTampilan(drTELUSUR.Item("Tanggal_Diterima"))
            Else
                cmdTELUSUR2 = New OdbcCommand(" SELECT * FROM tbl_Pembelian_BAST " &
                                              " WHERE Nomor_BAST = '" & NomorSJBAST & "' ", KoneksiDatabaseTransaksi)
                drTELUSUR2_ExecuteReader()
                drTELUSUR2.Read()
                TanggalSJBAST = TanggalFormatTampilan(drTELUSUR2.Item("Tanggal_Diterima"))
            End If
            AksesDatabase_Transaksi(Tutup)
            frm_Input_POPembelian.dgv_SJBAST.Rows.Add(NomorSJBAST, TanggalSJBAST)
        Next
        HapusFile(FolderRootApp & NamaFileDataSJBAST)
        frm_Input_POPembelian.BersihkanSeleksi_TabelSJBAST()
        '---------- BARIS AWAL YANG NANTINYA HARUS DIHAPUS SETELAH PROSES TRANSISI SISTEM DATA PEMBELIAN -----------------
        'NomorSJBAST_Bundelan = NomorSJBAST_Terseleksi
        'Dim NomorSJBAST = Kosongan
        'Dim TanggalSJBAST = Kosongan
        ...
        'DataSJBAST = IO.File.ReadLines(FolderRootApp & NamaFileDataSJBAST)
        'For Each KontenPerBaris In DataSJBAST
        '    NomorSJBAST = Microsoft.VisualBasic.Replace(KontenPerBaris, DoubleSlash, "")
        '    AksesDatabase_Transaksi(Buka)
        '    cmd = New OdbcCommand(" SELECT * FROM tbl_Pembelian_Invoice " &
        '                          " WHERE Nomor_SJ_BAST_Produk = '" & NomorSJBAST & "' ", KoneksiDatabaseTransaksi)
        '    dr_ExecuteReader()
        '    dr.Read()
        '    If dr.HasRows Then
        '        TanggalSJBAST = AmbilTanggal(dr.Item("Tanggal_SJ_BAST"))
        '    End If
        '    AksesDatabase_Transaksi(Tutup)
        '    frm_Input_POPembelian.dgv_SJBAST.Rows.Add(NomorSJBAST, TanggalSJBAST)
        'Next
        'HapusFile(FolderRootApp & NamaFileDataSJBAST)
        'frm_Input_POPembelian.BersihkanSeleksi_TabelSJBAST()
        '---------- BARIS AKHIR YANG NANTINYA HARUS DIHAPUS SETELAH PROSES TRANSISI SISTEM DATA PEMBELIAN -----------------
    End Sub
    ...
    Sub IsiTabelInvoice()
        ...
        For Each KontenPerBaris In DataInvoice
            ...
            End If
            AksesDatabase_Transaksi(Tutup)
            frm_Input_POPembelian.dgv_Invoice.Rows.Add(NomorInvoice, TanggalInvoice)
        Next
        HapusFile(FolderRootApp & NamaFileDataInvoice)
        frm_Input_POPembelian.BersihkanSeleksi_TabelInvoice()
    End Sub
    ...
    Private Sub DataTabelUtama_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataTabelUtama.CellClick
        ...
        KodeProjectProduk_Terseleksi = DataTabelUtama.Item("Kode_Project", Baris_Terseleksi).Value
        PembuatPO_Terseleksi = DataTabelUtama.Item("Pembuat_PO", Baris_Terseleksi).Value
        ...
        Attention_Terseleksi = DataTabelUtama.Item("Attention_", Baris_Terseleksi).Value
    ...
    Sub IsiValueHalamanCetak()
        AngkaPOPembelian_Cetak = AngkaPO_Terseleksi
        TampilkanHalamanCetak_POPembelian()
    End Sub
