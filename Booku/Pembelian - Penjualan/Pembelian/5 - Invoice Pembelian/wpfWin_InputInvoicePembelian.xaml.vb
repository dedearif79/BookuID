...
Public Class wpfWin_InputInvoicePembelian
    ...
    Sub ResetForm()
        ...
        dtp_TanggalDiterimaInvoice.Text = Kosongan
        dtp_TanggalSerahTerima.Text = Kosongan
        VisibilitasBarisPO(False)
    ...
    Private Sub dtp_TanggalInvoice_ValueChanged(sender As Object, e As SelectionChangedEventArgs) Handles dtp_TanggalInvoice.SelectedDateChanged
        If dtp_TanggalInvoice.Text <> Kosongan Then
            ...
            If FungsiForm = FungsiForm_TAMBAH Then
                dtp_TanggalDiterimaInvoice.SelectedDate = TanggalInvoice
            End If
            LogikaTarifPPN()
            LogikaTampilanKolomImporTermin()
            PenentuanKurs()
    ...
    Private Sub btn_PilihSupplier_Click(sender As Object, e As RoutedEventArgs) Handles btn_PilihMitra.Click
        ...
        '                     "Asal : " & AsalPembelian)
        If PembelianLokal Then
            If JenisRelasi = JenisRelasi_Afiliasi Then BukaFormListSupplierAfiliasi(txt_KodeSupplier, txt_NamaSupplier)
            If JenisRelasi = JenisRelasi_NonAfiliasi Then BukaFormListSupplierNonAfiliasi(txt_KodeSupplier, txt_NamaSupplier)
            If JenisRelasi = Pilihan_Semua Then BukaFormListSupplierDalamNegeri(txt_KodeSupplier, txt_NamaSupplier)
        End If
        If PembelianImpor Then BukaFormListSupplierLuarNegeri(txt_KodeSupplier, txt_NamaSupplier)
    End Sub
    ...
    Private Sub btn_PilihPO_Click(sender As Object, e As RoutedEventArgs) Handles btn_PilihPO.Click
        If KodeSupplier = Kosongan Then
            ...
            Return
        End If
        frm_ListPO.ResetForm()
        frm_ListPO.BersihkanSeleksi()
        frm_ListPO.Sisi = frm_ListPO.SIisi_POPembelian
        frm_ListPO.cmb_Mitra.Text = NamaSupplier
        frm_ListPO.lbl_FilterMitra.Enabled = False
        frm_ListPO.cmb_Mitra.Enabled = False
        frm_ListPO.MetodePembayaran = MetodePembayaran
        frm_ListPO.JalurMasuk = Form_INPUTINVOICEPEMBELIAN
        frm_ListPO.ShowDialog()
        EksekusiLogikaAdaPPh = False
        If frm_ListPO.DialogResult = Forms.DialogResult.Cancel Then Return
        ProsesIsiValueForm = True
        Kosongkan_TabelProduk()
        txt_NomorPO.Text = frm_ListPO.NomorPO_Terseleksi
    ...
    Sub TambahSJBAST_Manual()
        frm_InputSJBASTManual.ResetForm()
        If Not SupplierSebagaiPerusahaanLuarNegeri(KodeSupplier) Then
            frm_InputSJBASTManual.lbl_Nomor.Text = "Nomor SJ/BAST"
            frm_InputSJBASTManual.lbl_Tanggal.Text = "Tanggal SJ/BAST"
            frm_InputSJBASTManual.lbl_TanggalDiterima.Text = "Tanggal Diterima"
        Else
            frm_InputSJBASTManual.lbl_Nomor.Text = "Nomor BL/AWB"
            frm_InputSJBASTManual.lbl_Tanggal.Text = "Tanggal BL/AWB"
            frm_InputSJBASTManual.lbl_TanggalDiterima.Text = "Tanggal Fiat Muat"
        End If
        ProsesIsiValueForm = True
        frm_InputSJBASTManual.JalurMasuk = Form_INPUTINVOICEPEMBELIAN
        frm_InputSJBASTManual.dtp_TanggalDiterima.Value = TanggalInvoice
        frm_InputSJBASTManual.dtp_TanggalSJBAST.Value = TanggalInvoice
        ProsesIsiValueForm = False
        frm_InputSJBASTManual.ShowDialog()
        BersihkanSeleksi_TabelSJBAST()
    End Sub

    Sub TambahSJBAST_FromList()

        frm_ListSJBAST.ResetForm()
        frm_ListSJBAST.BersihkanSeleksi()
        frm_ListSJBAST.Sisi = frm_ListSJBAST.SisiPembelian
        frm_ListSJBAST.cmb_Mitra.Text = NamaSupplier
        frm_ListSJBAST.lbl_FilterMitra.Enabled = False
        frm_ListSJBAST.cmb_Mitra.Enabled = False
        If KunciTanggalInvoice = True Then
            frm_ListSJBAST.PilihTanggalDiterimaSJBAST = TanggalFormatTampilan(TanggalInvoice)
        Else
            frm_ListSJBAST.PilihTanggalDiterimaSJBAST = frm_ListSJBAST.TanggalSJBAST_Semua
        End If
        frm_ListSJBAST.JenisProduk_Induk = JenisProduk_Induk
        frm_ListSJBAST.JenisPPN = JenisPPN
        frm_ListSJBAST.PerlakuanPPN = PerlakuanPPN
        frm_ListSJBAST.JalurMasuk = Form_INPUTINVOICEPEMBELIAN
        If JumlahBarisSJBAST > 0 And
            ...
            frm_ListSJBAST.NomorPO_HarusSama = True
            frm_ListSJBAST.NomorPO_YangHarusDisamakan = NomorPO_TerakhirDitambahkan
        End If
        frm_ListSJBAST.ShowDialog()                                             '<---- Buka Form List SJ/BAST
        EksekusiLogikaAdaPPh = False
        NomorSJBAST = frm_ListSJBAST.NomorSJBAST_Terseleksi
        If NomorSJBAST = Kosongan Then Return
        NomorSJBAST_TerakhirDitambahkan = NomorSJBAST
        NomorPO_TerakhirDitambahkan = frm_ListSJBAST.NomorPO_Terseleksi
        TanggalSJBAST = frm_ListSJBAST.TanggalSJBAST_Terseleksi
        TanggalDiterimaSJBAST = frm_ListSJBAST.TanggalDiterima_Terseleksi
        Dim JenisSurat = frm_ListSJBAST.JenisSurat_Terseleksi
        ...
        Next
        If JumlahBarisSJBAST > 0 Then
            If frm_ListSJBAST.NomorPO_Terseleksi <> datatabelSJBAST.Rows(0)("Nomor_PO").ToString() Then
                MsgBox("Nomor Surat Jalan / BAST yang berbeda PO tidak dapat ditambahkan ke dalam daftar..!")
                Return
            End If
        End If
        datatabelSJBAST.Rows.Add(NomorSJBAST, TanggalSJBAST, TanggalDiterimaSJBAST, frm_ListSJBAST.NomorPO_Terseleksi, frm_ListSJBAST.OngkosKirim_Terseleksi)        '<-- Penambahan Baris SJ/BAST
            ...
            If Not IsNothing(KolomTerseleksi) Then
                If NomorUrutProduk_Terseleksi > 0 And KolomTerseleksi.DisplayIndex = COA_Produk.DisplayIndex Then
                    ...
                    If COAProduk_Terseleksi <> Kosongan Then
                        frm_ListCOA.COATerseleksi = COAProduk_Terseleksi
                        frm_ListCOA.txt_CariAkun.Text = COAProduk_Terseleksi
                    End If
                    frm_ListCOA.ListAkun = ListAkun_Pembelian
                    frm_ListCOA.ShowDialog()
                    COAProduk_Terseleksi = frm_ListCOA.COATerseleksi
    ...
    Private Sub txt_Diskon_Persen_PreviewTextInput(sender As Object, e As TextCompositionEventArgs) Handles txt_Diskon_Persen.PreviewTextInput
        HanyaBolehInputAngkaDesimalPlus_WPF(CType(sender, TextBox), e)
    End Sub
    ...
    Private Sub btn_Simpan_Click(sender As Object, e As RoutedEventArgs) Handles btn_Simpan.Click
        ...
        If StatusSuntingDatabase = True Then
            ...
            For Each row As DataRowView In datagridUtama.Items
                ...
                If AsalPembelian = AsalPembelian_Lokal Then
                    If JenisPPN = JenisPPN_Include Then
                        TabelSusunNBarisJurnal_JumlahDebet += CInt(Math.Round((HitungDPPUntukPPNInclude(TotalHargaPerItem, TarifPPN) + OngkosKirimAsset_PerItem)))
                    Else
                        TabelSusunNBarisJurnal_JumlahDebet += CInt(Math.Round(TotalHargaPerItem + OngkosKirimAsset_PerItem))
                    End If
                End If
            ...
            If StatusSuntingDatabase = True Then
                ...
                RefreshTampilanBukuPembelian()
                RefreshTampilanInvoicePembelian()
                RefreshTampilanBukuPengawasanHutangUsaha()
