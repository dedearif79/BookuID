# ============================================================
# Booku Remote Release Publisher with Obfuscation
# ============================================================
# Script ini akan:
# 1. Build project dalam mode Release
# 2. Obfuscate assemblies (Booku Remote.dll dan bcomm.dll)
...
$ProjectFile = Join-Path $ProjectDir "Booku Remote.vbproj"
...
Write-Host "    BOOKU REMOTE RELEASE PUBLISHER WITH OBFUSCATION" -ForegroundColor Magenta
Write-Host "============================================================" -ForegroundColor Magenta
...
if (-not (Test-Path $msbuildPath)) {
    ...
    Write-Warning "Visual Studio MSBuild tidak ditemukan, menggunakan dotnet CLI..."
    $useDotnet = $true
} else {
...
<Obfuscator>
  ...
  <Module file="`$(InPath)\Booku Remote.dll">
    <SkipNamespace name="Booku_Remote.My" />
...
"@
        ...
        if ($LASTEXITCODE -eq 0 -and (Test-Path (Join-Path $ObfuscatedDir "Booku Remote.dll"))) {
            ...
            Copy-Item -Path (Join-Path $ObfuscatedDir "Booku Remote.dll") -Destination $BuildOutputDir -Force
...
$exeFile = Join-Path $PublishDir "Booku Remote.exe"
...
Write-Host "   $FinalOutputDir\Booku Remote.exe" -ForegroundColor Yellow
